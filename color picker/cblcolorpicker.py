...
class CBLColorWheel(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.SVs = [(float(x) / self.piece_divisions, 1) for x in range(self.piece_divisions)] + [
            (1, float(y) / self.piece_divisions) for y in reversed(range(self.piece_divisions))]
        ...
        for r in range(self.piece_divisions):
            for t in range(self.pieces_of_pie):
                self.arcs.append(ColorArc(self.radius * (float(r) / self.piece_divisions),
                                          ...
                                          origin=self.origin,
                                          color=(float(t) / self.pieces_of_pie, self.SVs[self.SVidx + r][0],
                                                 self.SVs[self.SVidx + r][1], 1)))
    ...
    def on_touch_move(self, touch):
        ...
        r = self._get_touch_r(touch.pos)
        goal_SVidx = touch.ud['orig_SVidx'] - int(
            (r - touch.ud['anchor_r']) / (float(self.radius) / self.piece_divisions))
        ...
        if self._pinch_flag:
            if self._num_touches == 0:
                ...
                if self.SVidx > touch.ud['orig_SVidx']:
                    Clock.schedule_once(self.inertial_incr_SVidx, (Clock.get_time() - touch.ud['orig_time']) / (
                                self.SVidx - touch.ud['orig_SVidx']))
                if self.SVidx < touch.ud['orig_SVidx']:
                    Clock.schedule_once(self.inertial_decr_SVidx, (Clock.get_time() - touch.ud['orig_time']) / (
                                self.SVidx - touch.ud['orig_SVidx']))
...
class ColorArc(InstructionGroup):
    ...
    def __str__(self):
        return "r_min:" + str(self.r_min) + "r_max:" + str(self.r_max) + "theta_min" + str(
            self.theta_min) + "theta_max" + str(self.theta_max) + "color:" + str(self.color)
...
class CBLColorPicker(Widget):
    ...
    def button_callback(self, button_str):
        ...
        np_popup = Popup(content=np, title="Please choose a color value (0-255) for " + str(button_str),
